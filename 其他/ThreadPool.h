...
class ThreadPool {
  public:
    ...
    // 一个最简单的函数包装模板可以这样写(C++ 11)
    // 适用于任何函数(变参、成员都可以) template <class F, class... Args>
    // auto enqueue(F &&f, Args &&...args)
    //     -> decltype(declval<F>()(declval<Args>()...)) {
    //     return f(args...);
    // }
    // C++ 14更简单 template <class F, class... Args>
    // auto enqueue(F &&f, Args &&...args) {
    //     return f(args...);
    // }
...
auto ThreadPool::enqueue(F &&f, Args &&...args)
    ...
    std::future<return_type> res = task->get_future();
    {
        ...
        std::unique_lock<std::mutex> lock(queue_mutex); // 独占拿锁
